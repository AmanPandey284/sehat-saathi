...
import { ADAPTIVE_FIELD_LABELS } from '../../adaptiveQuestionBank';

export function labelField(field: string): string {
  if (ADAPTIVE_FIELD_LABELS[field]) {
    return ADAPTIVE_FIELD_LABELS[field];
  }
  const stripped = field.replace(/^adaptive_/, '');
  if (ADAPTIVE_FIELD_LABELS[stripped]) {
    return ADAPTIVE_FIELD_LABELS[stripped];
  }
  return field
    .replace(/([A-Z])/g, ' $1')
...
export function valueText(v: AnswerValue | undefined | null): string {
  ...
  if (typeof v === 'string') {
    ...
    if (lower === 'not_sure') return 'Not sure';
  }
  return String(v);
}
...
export function buildTimeline(
  ...
  for (const doc of documents) {
    events.push({
      ...
      date: doc.uploadedAt,
      title: doc.name,
      detail: `${doc.entities.length} extracted clinical item(s)`,
      source: 'DOCUMENT'
    });
  }
